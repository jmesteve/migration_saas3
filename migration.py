...
def empty(source_directory, destination_directory = None):
    tableContent = 'table_content'
    extension = '.dat'
    path_change = tableContent +"_" + source_directory[0] + extension
    try:
        ...
        if dataSource:
            for line in dataSource:
                try:
                    ...
                    print "not a number"
            shutil.copy(dir_source + 'toc.dat',dir_tmp + 'toc.dat')
            dir_exception = source_directory[0] + "_exception/"
            list = listdir(dir_exception)
            for line in list:
                file, extension = path.splitext(line)
...
def create_old(source, destination, source_directory, destination_directory):
    extension = '.dat'
    try:
...
def empty_old(source_directory, destination_directory):
    tableContent = 'table_content'
    extension = '.dat'
    try:
